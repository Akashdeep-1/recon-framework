...
STAGE_RETRIES="${STAGE_RETRIES:-1}"

# Scope and safety configuration
ENFORCE_STRICT_SCOPE="${ENFORCE_STRICT_SCOPE:-true}"

# Concurrency & Performance defaults
DEFAULT_THREADS="${DEFAULT_THREADS:-50}"
DEFAULT_RATE_LIMIT="${DEFAULT_RATE_LIMIT:-150}"
PARALLEL_PASSIVE="${PARALLEL_PASSIVE:-true}"

# DNSX settings
DNSX_THREADS="${DNSX_THREADS:-$DEFAULT_THREADS}"
...
# HTTPX settings
HTTPX_THREADS="${HTTPX_THREADS:-$DEFAULT_THREADS}"
HTTPX_TIMEOUT="${HTTPX_TIMEOUT:-10}"
HTTPX_RATE_LIMIT="${HTTPX_RATE_LIMIT:-$DEFAULT_RATE_LIMIT}"
...
NUCLEI_RATE_LIMIT="${NUCLEI_RATE_LIMIT:-$DEFAULT_RATE_LIMIT}"
